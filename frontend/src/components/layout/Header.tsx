      ...
      <div className="flex items-center gap-6">
        ...
        <div className="hidden md:flex items-center gap-1 text-sm font-medium text-slate-400 bg-slate-900/50 border border-slate-800 p-1 rounded-xl">
          ...
          <button 
            ...
            className={`px-3 py-1.5 rounded-lg transition-all ${i18n.language === 'uz' ? 'bg-indigo-600 text-white shadow-md' : 'hover:text-white hover:bg-slate-800'}`}
          >
            UZ
          </button>
          <button 
            onClick={() => changeLanguage('ru')} 
            className={`px-3 py-1.5 rounded-lg transition-all ${i18n.language === 'ru' ? 'bg-indigo-600 text-white shadow-md' : 'hover:text-white hover:bg-slate-800'}`}
          >
            RU
          </button>
          <button 
            onClick={() => changeLanguage('en')} 
            className={`px-3 py-1.5 rounded-lg transition-all ${i18n.language === 'en' ? 'bg-indigo-600 text-white shadow-md' : 'hover:text-white hover:bg-slate-800'}`}
          >
            EN
          </button>
        </div>
          ...
          {isNotificationsOpen && (
            <div className="absolute right-0 mt-3 w-80 bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden z-50 animate-in fade-in slide-in-from-top-2">
              <div className="p-4 border-b border-slate-800 flex justify-between items-center bg-slate-950/50">
                <h3 className="font-semibold text-white">Xabarnomalar</h3>
                <span className="text-xs text-indigo-400 cursor-pointer hover:text-indigo-300">Barchasini o'qish</span>
              </div>
        ...
        <div className="relative pl-6 border-l border-slate-800" ref={profileRef}>
          ...
          >
            <div className="flex flex-col items-end hidden sm:flex">
              <span className="text-sm font-bold text-white group-hover:text-indigo-400 transition-colors">{user?.name || 'Admin User'}</span>
          ...
          {isProfileOpen && (
            <div className="absolute right-0 mt-3 w-56 bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden z-50 animate-in fade-in slide-in-from-top-2">
              <div className="p-4 border-b border-slate-800 bg-slate-950/50">
                ...
                <p className="text-xs text-slate-500 mt-0.5">Pro Tarif</p>
              </div>
              <div className="p-2">
                <Link to="/dashboard/settings" onClick={() => setIsProfileOpen(false)} className="flex items-center gap-3 px-3 py-2.5 text-sm text-slate-300 hover:text-white hover:bg-slate-800 rounded-xl transition-colors">
                  <Settings size={16} /> {t('dashboard.settings')}
                </Link>
                <Link to="/dashboard/subscription" onClick={() => setIsProfileOpen(false)} className="flex items-center gap-3 px-3 py-2.5 text-sm text-slate-300 hover:text-white hover:bg-slate-800 rounded-xl transition-colors">
                  <CreditCard size={16} /> {t('dashboard.subscription')}
                </Link>
              </div>
              <div className="p-2 border-t border-slate-800">
                <button onClick={logout} className="flex items-center gap-3 px-3 py-2.5 w-full text-left text-sm text-red-400 hover:text-red-300 hover:bg-red-500/10 rounded-xl transition-colors">
