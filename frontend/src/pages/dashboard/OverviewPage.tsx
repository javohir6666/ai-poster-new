...
export const OverviewPage: React.FC = () => {
  ...
  const statCards = [
    { title: t('dashboard.total_channels'), value: stats?.totalChannels || 0, icon: Users, color: "text-blue-400", bg: "bg-blue-500/10" },
    { title: t('dashboard.active_bots'), value: stats?.activeBots || 0, icon: Activity, color: "text-emerald-400", bg: "bg-emerald-500/10" },
    { title: t('dashboard.total_posts'), value: stats?.totalPosts || 0, icon: MessageSquare, color: "text-indigo-400", bg: "bg-indigo-500/10" },
    { title: t('dashboard.ai_interactions'), value: stats?.aiInteractions || 0, icon: Zap, color: "text-amber-400", bg: "bg-amber-500/10" },
  ];
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {statCards.map((stat, index) => (
          <Card key={index} className="border-slate-800 bg-slate-900/50 backdrop-blur-sm hover:bg-slate-900 transition-colors">
